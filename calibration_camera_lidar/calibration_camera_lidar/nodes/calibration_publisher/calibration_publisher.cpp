...
static ros::Publisher camera_info_pub;    //发布camera_info消息的发布者
static ros::Publisher projection_matrix_pub;    //发布相机到lidar的tf
//状态标志位：用于判断runtime maneger中是否勾选相关选项
static bool isRegister_tf;    //注册相机到lidar的tf
static bool isPublish_extrinsic;    //是否发布外参
static bool isPublish_cameraInfo;   //是否发布相机消息

static std::string camera_id_str_;    
static std::string camera_frame_;
static std::string target_frame_;

static bool instrinsics_parsed_;     //内参语法检查标志位
static bool extrinsics_parsed_;      //外参语法检查标志位

static sensor_msgs::CameraInfo camera_info_msg_;    //相机信息
static autoware_msgs::ProjectionMatrix extrinsic_matrix_msg_;    //外参消息
...
{
	ros::init(argc, argv, "calibration_publisher");    //初始化ROS节点
	ros::NodeHandle n;    //句柄
